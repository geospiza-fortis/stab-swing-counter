...
def train_sliding_classifier(input, input_full, output, templates, window):
    input = Path(input)
    output = Path(output)
    output.mkdir(parents=True, exist_ok=True)
...
def evaluate(clf, labels, data, frames, video, output, batch_num, plot=False):
    X = np.array(data)
    y = clf.predict_proba(X)
    y_label = [labels[str(p)] for p in clf.predict(X)]
...
def evaluate_sliding_model(input, output, model_input, templates, window, batch_size):
    input = Path(input)
    model_input = Path(model_input)
    output = Path(output)
    output.mkdir(parents=True, exist_ok=True)
    ...
    for path in tqdm(sorted(Path(input).glob("**/*.png"))):
        if len(data) < batch_size:
            frame = cv.imread(str(path), cv.IMREAD_GRAYSCALE)
            feature = np.array(
    ...
    y, y_label = evaluate(clf, labels, data, frames, video, output, batch_num, True)
    pred_prob.append(y)
    pred_label.append(y_label)
